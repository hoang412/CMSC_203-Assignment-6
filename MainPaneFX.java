...
public class MainPaneFX {
	...
	MainPaneFX(double CANVAS_WIDTH, double CANVAS_HEIGHT) {
	    ...
	    optimizeButton = new Button("Optimize the problem");
	    optimizeButton.setTooltip(new Tooltip("Minimize the time"));
	    ...
	    optimizeButton.setOnAction(event -> {
	    	graphCanvas.setVisible(false);
			 ...
			 Optimum opt=null;
			 choice = askForFunction("Optimize the following problem: "+optimizerManager.toString());
			 if(choice != 0) {
				 optimizerManager.setFunctionChoice(choice);
				 try {
					 left = askForExtent("Enter the left extent of the function domain");
					 // check if the left extent less than 0 or not. 
					 while (left < 0) {
						 new Alert(Alert.AlertType.ERROR, "The left extent must me greater than or equal to 0").showAndWait();
						 left = askForExtent("Enter the left extent of the function domain"); 
					 }
					 ...
					 alert.setHeaderText("Optimum:");
					 alert.setContentText(optimizerManager.answerString(choice, 
			    	 ...
			    	 alert.setContentText("No entry: exiting");
	...
	private int askForFunction(String str) {
		...
		do {
			//try {
			Alert alert = new Alert(AlertType.INFORMATION);
			 alert.setHeaderText("The problem to be optimized:");
			 alert.setContentText(optimizerManager.toString());
			 alert.showAndWait();
			 returnVal = 1; // hardwired for Assignment 6  
	...
	private double askForExtent(String str) throws NullPointerException {
		...
		do {
			 try {
				 ...
				 dialog.setHeaderText("Enter extent as a double:");
				 dialog.setContentText(str);
				 Optional<String> result = dialog.showAndWait();
				 returnVal = Double.parseDouble(result.get());
		 		...
		 		Alert alert = new Alert(AlertType.ERROR);
		    	alert.setContentText("Input Error: "+e);
		    	alert.showAndWait();
		    	//JOptionPane.showMessageDialog(null, "Input Error - "+e); 
		 	 	error = true;
		 	}
